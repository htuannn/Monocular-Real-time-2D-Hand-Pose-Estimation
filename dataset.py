...
class FreiHand(Dataset):
	def __init__(self, config, n_sample=1, set_type="train", transform=None):
			##initialize path to the image folder
			self.device=config["device"]
			self.n_sample=n_sample
			self.imgs_path=os.path.join(config["data_dir"],"training/rgb")
			self.imgs_list=np.sort(os.listdir(self.imgs_path))
			...
			keypoints = [projectPoints(x,y)/ RAW_IMG_SIZE for (x,y) in list(zip(self.xyz_coor,self.K_matrices))] 

			train_list, rem_list, train_key, rem_key= train_test_split(self.imgs_list[:len(keypoints)*n_sample], keypoints * n_sample, train_size=0.8, random_state=18)
	...
	def __len__(self):
		...
		"""
		return len(self.keypoints)*self.n_sample
